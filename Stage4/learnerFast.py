...
dataset = numpy.loadtxt(open("data.txt"),delimiter=",")
...
for i in range(0, len(pred)):
    ...
    prod1 = prodPairs[i][0]
    prod2 = prodPairs[i][1]
    pld1 = prod1["Product Long Description"][0]
    pld2 = prod2["Product Long Description"][0]
    pn1 = prod1["Product Name"][0]
    pn2 = prod2["Product Name"][0]
    strsmall = ""
    strbig = ""
    if len(pld1) < len(pld2):
        strsmall = pld1
        strbig = pld2
    else:
        strsmall = pld2
    ...
    if pred[i] == 1:
        if ("Color" in prod1 and "Color" in prod2 and prod1["Color"][0] != prod2["Color"][0]):
            pred[i] = 0
        elif ("Product Long Description" in prod1 and "Product Long Description" in prod2):
            if (len(strsmall) < (len(strbig) * 0.25)) and (simfunctions.jaccard(pn1.split(), pn2.split()) < 0.35):
                pred[i] = 0
